...
export default function TabContent3() {
    ...
    async function fetchData() {
        try {
            const filteredOrders = RunningOutList.filter(item => item.quantity = 0);
            setOutOfStockList(filteredOrders);
        } catch (error) {
        ...
        <>
            <div className="w-[70vw] flex gap-8 mx-auto flex-wrap">
                ...
                {outOfStockList.length === 0 ? (
                    ...
                    outOfStockList.slice(0, 5).map((item) => (
                            ...
                            <div className="flex w-[12rem] text-sm flex-col gap-4 p-3 min-h-[8rem] bg-white rounded-xl">
                                ...
                                <div className="flex flex-col flex-1">
                                    ...
                                    <div>
                                        ...
                                        <Link href={"/Dashboard/inventory/cardDetails"} className="flex rounded-full py-2 items-center justify-center bg-secondary-500 text-white">
                                            View details
                                        </Link>
                                    </div>
                                </div>
